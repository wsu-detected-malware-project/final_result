...
namespace user_program.FormAll {
    public partial class Form1 : Form
    {
        ...
        private void Form1_Activated(object sender, EventArgs e)
        {
            ...
            string pathMapPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "path_map.csv");
            if (File.Exists(resultPath))
            {
                try
                {
                    ...
                    string fileName = $"result_{DateTime.Now:yyyy-MM-dd_HHmmss}.txt";
            ...
            if (File.Exists(pathMapPath)) 
            {
                try
                {
                    File.Delete(pathMapPath);
                }
                catch (Exception ex)
                {
                    MessageBox.Show("path_map.csv 삭제 중 오류 발생:\n" + ex.Message);
                }
            }
        }
